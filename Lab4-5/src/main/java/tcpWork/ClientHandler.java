...
public class ClientHandler extends Thread{
    ...
    private void showBalance(Object obj)
        ...
        int ind = bnk.findMetroCard(op.getCardSerialNumber());
        if (ind >= 0) {
            os.writeObject("Card : " + op.getCardSerialNumber() + " balance: "
                    + bnk.getStore().get(ind).getBalance());
            os.flush();
        } else {
            os.writeObject("Cannot Show Balance for Card: " + op.getCardSerialNumber());
        }
    }
